...
std::vector<std::tuple<const char*, std::string, int>> queuedFonts;

void addFontToQueqe(const char* name, std::string path, int size) {
	queuedFonts.emplace_back(name, path, size);
}

void createFont(const char* name, std::string path, int size) {
...
void toggleFPS(Instance* s, Color textColor = { 0,0,0,255 }) {
	...
	if (!labelFPS) {
		...
		new ChangedSignal(accurateFPS, []() {
			static int last = 0;
			if (last != accurateFPS) {
				labelFPS->Text = std::to_string(accurateFPS) + " FPS";
			}
		});
